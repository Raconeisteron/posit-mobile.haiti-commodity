...
public class CommodityMessage {
	...
	public static final String TAG = "CommodityMessage";
	...
	public static final String MESSAGE_BENEFICIARY_ID = CommodityAttributeManager.MESSAGE_BENEFICIARY_ID;  // Row Id in Beneficiary table
	public static final String MESSAGE_TEXT = CommodityAttributeManager.MESSAGE_TEXT;
	public static final String MESSAGE_STATUS = CommodityFind.MESSAGE_STATUS; 
	...
	public static final String MESSAGE_CREATED_AT = CommodityAttributeManager.MESSAGE_CREATED_AT;
	public static final String MESSAGE_SENT_AT = CommodityAttributeManager.MESSAGE_SENT_AT;
	public static final String MESSAGE_ACK_AT = CommodityAttributeManager.MESSAGE_ACK_AT;
	...
	public CommodityMessage(int messageId, int beneficiaryId, int msgStatus,  
	 ...
	 *
	 */
	public CommodityMessage(String smsText) {
		Log.i(TAG, "Creating from smstext:" + smsText);
		String[] msgparts = smsText.split(CommodityAttributeManager.PAIRS_SEPARATOR);
//		for (int k = 0; k < msgparts.length; k++) {
//			Log.i(TAG, "msgpart " + k + " :" + msgparts[k]);
//		}
		String[] firstPair = msgparts[0].split(CommodityAttributeManager.ATTR_VAL_SEPARATOR);  // AV=id
		String[] thirdPair = msgparts[2].split(CommodityAttributeManager.ATTR_VAL_SEPARATOR);  // mi=mid
		...
		if (avId < 0 && avId != UNKNOWN_ID) {
			...
			beneficiaryId = avId;
//			messageId =  CommodityDbHelper.UNKNOWN_ID;
			messageId =  msgId;
			existing = true;
		} 
...
//			Log.i(TAG, "msgpart " + k + " :" + msgparts[k]);
			smsMessage += msgparts[k] + CommodityAttributeManager.PAIRS_SEPARATOR;
		}
	...
	public static void createTable(ConnectionSource connectionSource) {
		try {
			TableUtils.createTable(connectionSource, CommodityMessage.class);
	 ...
	 * @return
	 */
	public static int clearTable(Dao<CommodityMessage, Integer> dao) {
		Log.i(TAG, "Clearing Message Table");
		int count = 0;
		try {
			DeleteBuilder<CommodityMessage, Integer> deleteBuilder = dao.deleteBuilder();
	 ...
	 * @return
	 */
	public static int createMessage (Dao<CommodityMessage, Integer> daoMsg, 
			Dao<CommodityFind, Integer> daoFind, 
			CommodityMessage acdiVocaMsg, int beneficiary_id, int msgStatus) {
...
//			try {
				success = CommodityFind.updateMessageStatus(daoFind, beneficiary_id, acdiVocaMsg.id, msgStatus);
//			} catch (SQLException e) {
...
//	 * @return
//	 */
//	public static List<CommodityMessage> fetchAllByStatus(Dao<CommodityMessage, Integer> dao, int statusFilter) {
//		int msg_status = mapSearchFilterToMessageStatus (statusFilter);
//
//		List<CommodityMessage> list = null;
//		CommodityMessage avMsg = null;
//		try {
//			QueryBuilder<CommodityMessage, Integer> queryBuilder = dao.queryBuilder();
//			if (msg_status != 0) {
//				Where<CommodityMessage, Integer> where = queryBuilder.where();
////				where.eq("msgStatus", msg_status);
//				where.eq(MESSAGE_STATUS, msg_status);
//			}
//			PreparedQuery<CommodityMessage> preparedQuery = queryBuilder.prepare();
...
//	}
	...
	private static int mapSearchFilterToMessageStatus (int searchFilter) {
		switch (searchFilter) {
		case CommoditySearchFilterActivity.RESULT_SELECT_PENDING:
			return CommodityMessage.MESSAGE_STATUS_PENDING;
		case CommoditySearchFilterActivity.RESULT_SELECT_SENT:
			return CommodityMessage.MESSAGE_STATUS_SENT;
		case CommoditySearchFilterActivity.RESULT_SELECT_ACKNOWLEDGED:
			return CommodityMessage.MESSAGE_STATUS_ACK;
		default:
			return CommodityMessage.MESSAGE_STATUS_UNSENT;
		}
	}
	 ...
	 * @return
	 */
	public static CommodityMessage fetchById (Dao<CommodityMessage, Integer> dao, int msgId) {		
	 ...
	 */
	public static ArrayList<CommodityMessage> fetchAllByStatus(Dao<CommodityMessage, Integer> dao, int statusFilter) {
		int msg_status = mapSearchFilterToMessageStatus (statusFilter);
...
//				where.eq("msgStatus", msg_status);
		...
		ArrayList<CommodityMessage> acdiVocaMsgs =  new ArrayList<CommodityMessage>();

		// Create an ArrayList that adds message header messages.
		Iterator<CommodityMessage> it = list.iterator();
		while (it.hasNext()) {
			CommodityMessage msg = it.next();
	 ...
	 * @return
	 */
	public static boolean updateStatus(Dao<CommodityMessage, Integer> dao, int beneficiary_id, int msg_id, int status ) {
...
//		+ " " + new Time(System.currentTimeMillis()).toString();
		...
		try {
			...
			if (avMsg != null) {
				...
				if (status == CommodityMessage.MESSAGE_STATUS_SENT) {
					avMsg.message_sent_at = now;
				} else if (status == CommodityMessage.MESSAGE_STATUS_ACK) 
...
//	 * @return
//	 */
//	public static boolean updateMessageStatusForNonBulkMessage(Dao<CommodityMessage, Integer> dao, CommodityMessage acdiVocaMsg, int status) {
...
////		
////		try {
////			Dao<CommodityFind, Integer> dao = getCommodityFindDao();
////			CommodityFind avFind = dao.queryForId(beneficiary_id);
////			if (avFind != null) {
...
//			try {
//				result = CommodityMessage.updateStatus(getCommodityMessageDao(), beneficiary_id, msg_id, status);
//			} catch (SQLException e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//			}
...
//	}
	...
	public String toString() {
		...
		if (beneficiaryId != UNKNOWN_ID) {
			...
			+ CommodityAttributeManager.PAIRS_SEPARATOR
			+ CommodityAttributeManager.ABBREV_MSG_NUMBER_SLASH_SIZE
			+ CommodityAttributeManager.ATTR_VAL_SEPARATOR
			+ getNumberSlashBatchSize()
			+ CommodityAttributeManager.PAIRS_SEPARATOR
			+ CommodityAttributeManager.ABBREV_MESSAGE_ID
			+ CommodityAttributeManager.ATTR_VAL_SEPARATOR 
			+ messageId 
			+ CommodityAttributeManager.PAIRS_SEPARATOR
			+ getSmsMessage();
		...
		return message;
		//return msgHeader + CommodityAttributeManager.PAIRS_SEPARATOR + smsMessage;
	}
}
